...
public class PatternController {
    ...
    private byte[] generateMidiBytes(MidiPattern pattern) throws Exception {
        ...
        for (Note note : pattern.getNotes()) {
            ...
            on.setMessage(ShortMessage.NOTE_ON, 0, note.getMidiNoteNumber(), 90);
            track.add(new MidiEvent(on, tick));

            tick += (long) (note.getDuration() * 4);

            ShortMessage off = new ShortMessage();
            off.setMessage(ShortMessage.NOTE_OFF, 0, note.getMidiNoteNumber(), 0);
